...
class GUILogFilter(logging.Filter):
    """过滤 GUI 中的低价值调试噪声，保留关键状态和 API 摘要。"""
    _NOISY_NAMES = {
        "logic.navigator",
    }
    _NOISY_INFO_FRAGMENTS = (
        "当前页面:",
        "页面状态:",
        "校区下拉框:",
        "校区选项",
    ...
    def __init__(self):
        super().__init__()
        level_name = _get_config_attr("GUI_LOG_LEVEL", "INFO").upper()
    ...
    def filter(self, record):
        ...
        if "[API]" in message or record.name == "core.api_client":
            return True
        if record.name in self._NOISY_NAMES:
            return not any(fragment in message for fragment in self._NOISY_INFO_FRAGMENTS)
        return True
...
def attach_gui_handler(callback):
    ...
    _gui_handler.addFilter(GUILogFilter())
