...
| 更新 | 说明 |
| --- | --- |
| API 并发扫描 | 高级设置新增 API 并发选项，可选 `1`、`5`、`10`、`15`、`20`，默认 `10`。`1` 为逐个座位扫描，速度最慢但最稳；并发越高越快，但越容易触发超时。 |
| 超时策略优化 | 座位时间接口取消长时间等待和二次重试。实测部分座位就是会拿不到结果，继续等待只会拖慢整体扫描进度，现在跳过当次超时继续生成方案。 |
| 双进度条 | 扫描时弹出进度窗口，上方显示总体房间进度，下方显示当前房间座位进度，直观看到当前扫描位置，不再面对静止的浏览器不知所措。 |
| 方案列表升级 | 单账号候选和多账号整套方案改为列表选择弹窗，支持筛选、排序、滚动查看和详情预览。多账号方案最多展示 50 套，跨房间开启时保留目标房间方案，避免推荐偏离优先房间。 |
| 模式切换修复 | 修复多账号切回单账号后重启程序，界面仍显示多账号账号列表但实际运行单账号模式的问题，现在切换后重启保持一致。 |
| 弹窗与主题 | 方案选择弹窗改为主界面 Qt 弹窗，跟随亮色/暗色主题，并主动置顶到前台。 |
| 驱动清理 | 增强退出和异常时的驱动清理，减少闪退后 `msedgedriver`/`chromedriver` 残留进程。 |

> 少量座位在扫描末尾仍可能超时。部分房间里存在临时损坏、社团专用或状态不可用的座位，这些座位是否固定无法确认，所以程序不会长期排除它们，只能硬着头皮扫描，跳过当次超时并继续生成方案。
> 
## v2.5.3主要更新
...
![暗色主界面](docs/screenshots/readme-v260-main-dark.png)
...
<img src="docs/screenshots/readme-v260-api-progress-warning.png" alt="暗色扫描进度" width="95%">
<br>暗色主题 · 含超时警告
</td>
<td width="50%" align="center">
<img src="docs/screenshots/readme-v260-api-progress-light.png" alt="亮色扫描进度" width="95%">
<br>亮色主题
</td>
</tr>
</table>
...
<tr>
<td width="50%" align="center">
<img src="docs/screenshots/readme-v260-multi-options.png" alt="多账号方案列表" width="95%">
<br>多账号方案列表 · 筛选排序
</td>
<td width="50%" align="center">
<img src="docs/screenshots/readme-v260-seat-options.png" alt="单账号候选列表" width="95%">
<br>单账号候选列表 · 详情预览
</td>
</tr>
</table>
