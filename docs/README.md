...
本文档适用于当前桌面项目 `D:\User\MC233\Desktop\LNU-LibSeat`。它和参考目录里的 `LNU-LibSeat-Automation` 是不同功能版本：当前项目以 PySide6 GUI、API 扫描规划、单账号逐段换座恢复和多账号分时段为核心。
...
| [用户使用指南](USER_GUIDE.md) | 普通用户 | 普通预约、单账号逐段、多账号分时段、跨房间和优先座位。 |
