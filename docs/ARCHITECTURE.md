...
| UI 层 | `ui/main_window.py`, `ui/panels/*`, `ui/widgets/*` | 展示配置、日志、帮助、状态栏和操作按钮。 |
| Worker 层 | `ui/workers/alloc_worker.py` | 调度登录、API 扫描、预约、取消、换座、恢复和通知。 |
...
| 模块 | 说明 |
| --- | --- |
| `ui/main_window.py` | 主窗口。处理首次帮助、主题切换、启动/停止、关闭防误关和恢复状态检测。 |
| `ui/workers/alloc_worker.py` | 核心后台流程。包含测试模式、多账号分段和单账号逐段逻辑。 |
| `ui/runtime_state.py` | 保存、读取、清理单账号恢复状态。 |
| `logic/api_planner.py` | API-only 扫描和推荐方案生成。 |
| `logic/booking_manager.py` | 查询“我的预约”、取消当前预约、返回座位图。 |
| `logic/booker.py` | Selenium 座位预约和验证码处理。 |
| `core/api_client.py` | LibSeat 后端接口请求。 |
| `core/desktop_notify.py` | 首次方案确认、换座确认、无座提醒、恢复三选项弹窗。 |
